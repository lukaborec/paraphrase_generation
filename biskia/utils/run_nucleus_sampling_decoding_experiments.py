...
parser.add_argument('--top_p', type=float, help='top_p parameter for nucleus sampling', default=0.8)
...
print(f"Running nucleus sampling experiments with top_p={top_p} and num_paraphrases={num_paraphrases}")
    ...
    for data_loader in dataloaders:
        ...
        for current_step, (inputs, outputs) in enumerate(dataloader):
            try:
                ...
                semantics = inputs[0]['semantics'][:3] # dataloader_validate has different structure for some reason
            label = outputs

            # Feed instruction into model to generate paraphrases
            if type(inputs) == dict():
                inputs = [inputs]
            if data_loader[0] != "train": # Val/test format things differently
                inputs = inputs[0]

            paraphrases = model.nucleus_sampling([inputs], DEVICE, top_p, num_paraphrases)

            processed_paraphrases = []
            for p in paraphrases:
                p = torch.stack([t.squeeze() for t in p])
                processed_paraphrases.append(p)
            paraphrases = processed_paraphrases

            # Clean paraphrases for input into the interpreter model
            prepared_paraphrases = []
            for p in paraphrases:
                try:
                    pp = {"instruction" : prepare(p[0])}
                except:
                    pp = {"instruction" : prepare(p)}
                prepared_paraphrases.append(pp)
        ...
        file_name = f"{model_name}_nucleus_sampling_{top_p}_{data_loader[0]}"
